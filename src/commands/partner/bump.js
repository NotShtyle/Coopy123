  ...
  if (now - lastDate[message.guild.id] > cooldown || ignoreCooldown) {
    ...
    client.database.all('SELECT * FROM settings').then(async row => {
      message.guild.fetchInvites().then(async invites => {
        if (row.length - 1 <= 0) {
          client.embed.send(message, { desc: 'There are no other guilds for your advertisement to go, `e!invite` and setup the bot on other guilds before trying again.' })
          return
        }
    ...
    client.embed.send(message, { desc: `You must wait **${remaining}** before you can use this command again. :clock:` })
  }
}
...
function bumpLogic (client, message, row, invite) {
  ...
  client.embed.send(message, { desc: `Bumped to ${row.length - 1} guild(s)!` })
